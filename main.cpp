...
Tensor scaled_dot_attention(Tensor q, Tensor k, Tensor v) { 
    ...
    Tensor product = matmul(q, k.transpose(-1, -2), scale);    
...
struct CLSToken : nn::Module {
    ...
    Tensor forward(Tensor sequence) const {
        auto cls = expand(weight, sequence.size(0), -1, -1);
...
int main() {   
    nn::Parameters parameters; parameters.initialize("B-16-imagenet1k");
    ViT model(float32, 12, 16, 3, 384, 768, 3072, 12, 1000); model.initialize(parameters);
